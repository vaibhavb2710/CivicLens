...
class TrashBinDetectionResult {
  ...
  @override
  String toString() {
    return 'TrashBinResult(isTrash: $isTrash, GPS: ${location != null ? 'Yes' : 'No'})';
  }
}

class TrashBinDetectionService {
  ...
  static void _log(String message) {
    debugPrint('🗑️ TrashBinService: $message');
  }
  ...
  static Future<String?> _discoverServer() async {
    ...
    _log('🔍 Discovering Trash Bin FastAPI server on network...');
    ...
    for (int i = 0; i < possibleIPs.length; i += batchSize) {
      ...
      for (String? result in results) {
        if (result != null) {
          ...
          _log('🎯 Trash Bin FastAPI server found: $result');
          return result;
        }
      }
    ...
    _log('❌ No Trash Bin FastAPI server discovered on network');
    return null;
  }

  static Future<String?> _testServerIP(String ip) async {
    try {
      String testUrl = 'http://$ip:8002'; // Port 8002 for trash bin
  ...
  static Future<TrashBinDetectionResult> detectTrashBin({
    File? imageFile,
    Uint8List? webImageBytes,
    String? imageName,
  }) async {
    try {
      _log('🔍 Starting trash bin detection...');

      String? serverUrl = await _discoverServer();
      if (serverUrl == null) {
        return TrashBinDetectionResult(
          isTrash: false,
          confidence: 0.0,
          detectionClass: 'no_server_found',
          error:
              'No Trash Bin FastAPI server found. Ensure server is running on port 8002.',
        );
      }
      ...
      if (kIsWeb && webImageBytes != null) {
        request.files.add(
          http.MultipartFile.fromBytes(
            'file',
            webImageBytes,
            filename: imageName ?? 'trashbin_detection.jpg',
      ...
      _log('🚀 Sending trash bin detection request...');
      ...
      _log('❌ Trash bin detection completely failed: $e');
      ...
      return TrashBinDetectionResult(
        ...
        error: 'Trash bin detection failed: ${e.toString()}',
      );
    }
  }

  static TrashBinDetectionResult _parseTrashBinResponse(
    String body,
    ...
    try {
      _log('📊 Parsing trash bin response...');
      ...
      if (data.containsKey('isTrash')) {
        isTrash = data['isTrash'] == true;
      }
      ...
      _log('🎯 Trash bin parsing complete:');
      _log('   Trash bin detected: $isTrash');
      ...
      _log('❌ Trash bin parsing failed: $e');
      return TrashBinDetectionResult(
        isTrash: false,
        confidence: 0.0,
  ...
  static Future<TrashBinDetectionResult> captureAndDetect() async {
    try {
      _log('📷 Opening camera for trash bin detection...');
      ...
      if (kIsWeb) {
        ...
        return await detectTrashBin(
          webImageBytes: bytes,
          imageName: pickedFile.name,
        );
      } else {
        _log('📊 Mobile image path: ${pickedFile.path}');
        return await detectTrashBin(imageFile: File(pickedFile.path));
      }
    } catch (e) {
  ...
  static Future<String?> refreshServerDiscovery() async {
    _discoveredServerUrl = null;
    _lastDiscovery = null;
    _log('🔄 Forcing trash bin server rediscovery...');
